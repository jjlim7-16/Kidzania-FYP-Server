...
router
  ...
  .get((req, res) => {
    const sql = `Select booking_id, rfid, b.session_date, se.session_start, se.session_end, st.station_name,
...
router.get("/:bookingID", function(req, res) {
  ...
  const sql = `Select booking_id, b.session_date, se.session_start, se.session_end, st.station_name,
...
router.put("/updateStatus/:bookingID", (req, res) => {
  ...
  const sql = `update booking_details set booking_status =?, time_in = ? where booking_id = ?`;
  const val = [bookingData.booking_status, bookingData.time_in, bookingID];
  pool.getConnection().then(function(connection) {
    connection
      .query(sql, val)
      .then(rows => {
  ...
  pool.getConnection().then(function(connection) {
    connection
      ...
      .then(rows => {
        ...
        qNum =
          alphas.charAt(parseInt(bookingData.station_id - 1)) +
          zerofill(4, qNum);
        ...
        date =
          ...
          "-" +
          date.getDate();
        const bookingDetails_val = [
          parseInt(bookingData.session_id),
          date,
          parseInt(bookingData.station_id),
          bookingData.role_id,
          bookingData.rfid,
          qNum,
          bookingData.status
...
router.put("/cancelBooking", (req, res) => {
  ...
  const sql = `UPDATE booking_details SET booking_status = ? WHERE session_id = ? AND rfid = ?`;
  const val = [details.status, parseInt(details.session_id), details.rfid];
  pool.getConnection().then(function(connection) {
    connection
      .query(sql, val)
      .then(() => {
...
router.get("/rfid/:rfid", (req, res) => {
  ...
  const sql = `SELECT bd.booking_id,sr.role_name,bd.session_id, bd.session_date, bd.station_id, bd.role_id,
...
router.get("/getbookinglist/:stationId", (req, res) => {
  const stationidStr = req.params.stationId;
  const stationid = parseInt(stationidStr);
  const sql = `SELECT b.booking_id,ase.session_id,s.station_name, ase.session_date, sr.role_name,b.time_in, se.session_start, se.session_end,  b.booking_status, b.rfid,b.queue_no
  ...
  pool.getConnection().then(function(connection) {
    connection.query(sql, [stationid, stationid]).then(rows => {
      res.json(rows);
    });
    connection.release();
  });
